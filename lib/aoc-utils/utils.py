...
def get_aoc_headers():
    ...
    if script_environment == "github_actions":
        ...
        try:
            ...
            aoc_session_cookie = input(
                f"{A.CYAN}Enter your AoC session cookie {A.MAGENTA}(instructions on how to get it can be found in README.md):{A.RESET}\n "
            )
...
def fetch_problem_data(part, year, day, headers):
    ...
    if part == 2:
        problem_url += "#part2"
    problem_name, problem_text, example_input, example_solution = extract_problem_info(
        part, soup
    )
...
def create_solution_file(part, dirname, lang_choice, example_solution):
    ...
    os.system(
        f"cp templates/{lang_choice}/solution.{lang_extension} {solution_file_path}"
    )
...
def submit(answer):
    ...
    problem_url = f"https://adventofcode.com/{year}/day/{day}"
    post_data = {"level": part, "answer": str(answer)}
